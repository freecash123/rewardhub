'use client';
import { useState, useEffect, createContext, context, ReactNode } from 'react';
import { authApi, getToken, setToken, removeToken } from '@/lib/api';
interface User { id: string; email: string; username: string; balance: number; referralCode: string; role: string; }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    setToken(res.token); setUser(res.data.user); return res;
  };
